...
var svginter = __webpack_require__(2); 
var svgdraw = SVG("drawing_svgjs_intersection").size(300, 300);

var posX=[], posY=[], deltaX=[], deltaY=[], dirs=[], circles=[];
var frameCount=0, frameRate=0, circleCount=0;
var rectFrame = {
    left:   0,
    top:    0,
    right:  svgdraw.width(),
...
var frameLines = []
frameLines.push(svgdraw.line(rectFrame.left, rectFrame.top, rectFrame.left, rectFrame.bottom).attr({stroke:"black", 'stroke-width': 2}));
frameLines.push(svgdraw.line(rectFrame.left, rectFrame.bottom, rectFrame.right, rectFrame.bottom).attr({stroke:"black", 'stroke-width': 2}));
frameLines.push(svgdraw.line(rectFrame.right, rectFrame.bottom, rectFrame.right, rectFrame.top).attr({stroke:"black", 'stroke-width': 2}));
frameLines.push(svgdraw.line(rectFrame.right, rectFrame.top, rectFrame.left, rectFrame.top).attr({stroke:"black", 'stroke-width': 2}));
///Frame Rate
...
function createCircle(i){
    var posX = Math.floor((Math.random() * svgdraw.width()) - circleSize);
    var posY = Math.floor((Math.random() * svgdraw.height()) - circleSize);
    if(posX < 0) posX=0;
    if(posY < 0) posY=0;
    var circle = svgdraw.circle(circleSize).attr({fill: colors[i]}).move(posX, posY);
    return {
        posX, posY, circle
    };
}

function changeDelta(i, dir){
    if(dir == 0)
        deltaX[i] = 1, deltaY[i] = 1;
    else if(dir == 1)
        deltaX[i] = 1, deltaY[i] = -1;
    else if(dir == 2)
        deltaX[i] = -1, deltaY[i] = -1;
...
function intersectRect(r1, r2) {
    var r1 = r1.node.getBoundingClientRect();    //BOUNDING BOX OF THE FIRST OBJECT
    var r2 = r2.node.getBoundingClientRect();    //BOUNDING BOX OF THE SECOND OBJECT
...
function moveCircle(i){
    circles[i].move(posX[i] += deltaX[i], posY[i] += deltaY[i]);
}
